...
#Plot 4
png(file="plot4.png",bg="transparent") # Default is 480 x 480
par(mfcol=c(2,2))
# GAP vs datetime
...
legend("topright",legend=c("Sub_metering_1","Sub-metering_2","Sub_metering_3"),lty=1,col=c("black","red","blue"),bty="n")
#V vs datetime
with(workingdata,plot(Voltage~datetime,ylab="Voltage",xlab="datetime",type="n"))
with(workingdata,lines(Voltage~datetime))
#GRP vs datetime 
with(workingdata,plot(Global_reactive_power~datetime,ylab="Global_reactive_power",xlab="datetime",type="n"))
with(workingdata,lines(Global_reactive_power~datetime))
dev.off()
